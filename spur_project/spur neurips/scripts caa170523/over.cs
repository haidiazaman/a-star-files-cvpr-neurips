...
public class over : Randomizer
{
    ...
    protected override void OnIterationStart()
    {
        ...
        if (randomisePos==1)
        {
            ...
            mainCamera.transform.rotation = Quaternion.Euler(cameraRotationX.Sample(),180,cameraRotationZ.Sample());
            mainCamera.fieldOfView = cameraFOV.Sample();
        }
        else
        {
            mainCamera.transform.position = new Vector3(cameraCirclePos.x,cameraCirclePos.y,-cameraDepthZ.Sample());
            mainCamera.transform.rotation = Quaternion.Euler(cameraRotationX.Sample(),0,cameraRotationZ.Sample());
            mainCamera.fieldOfView = cameraFOV.Sample();
        }
