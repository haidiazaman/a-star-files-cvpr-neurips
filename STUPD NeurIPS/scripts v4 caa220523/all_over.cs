...
public class all_over : Randomizer
{
    ...
    {   
        ...
        {
            spawnPoint = currentInstance.transform.position + Random.insideUnitSphere * spawnRadius.Sample();
            while (Physics.CheckSphere(spawnPoint,spawnCollisionCheckRadius.Sample()))
            {
                spawnPoint = currentInstance.transform.position + Random.insideUnitSphere * spawnRadius.Sample();
            }
